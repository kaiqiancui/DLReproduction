...
def main(args):
    ...
    # 用于获取原始模型，去除各种包装器(如DistributedDataParallel, DataParallel等)
    # 如果是编译后的模型，返回原始模型版本
    ...
    # 使用 functools.partial 创建一个新的函数
    # 预设了一些固定参数，包括：
    # proportion_empty_prompts: 空提示词的比例
    # text_encoders: 文本编码器列表
    # tokenizers: 分词器列表
    # 这样后续只需要传入 batch 数据即可
    ...
    # 初始化一个标志位，表示是否覆盖了最大的下训练步数
    # gradient_accumulation_steps 梯度累积步数：累计多少个小批量梯度之后才进行一次参数更新
    ...
    # 在分布式训练中训练参数可能会改变大小，需要重新计算各个训练参数
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            #这个with用于梯度累积
                ...
                # pixel_values.shape[0]: 获取批次大小
                # range(0, pixel_values.shape[0], 8): 以步长 8 创建索引序列
                # pixel_values[i : i + 8]: 每次取 8 张图片进行处理
                # vae.encode(): VAE 的编码器函数,将图像转换为潜在空间表示
                # latent_dist: 返回的是潜在空间的分布(通常是正态分布)
                # sample(): 从这个分布中采样得到具体的潜在表示
                # latents.append(): 将每批次的结果添加到列表中
                # 从此实现了分批编码
                latents = []
                for i in range(0, pixel_values.shape[0], 8):
                    latents.append(vae.encode(pixel_values[i : i + 8]).latent_dist.sample())
                
                # torch.cat(latents, dim=0): 将所有批次的潜在表示在第 0 维度上拼接起来
                # vae.config.scaling_factor: VAE 的缩放因子(通常为 0.18215)
                # 缩放是为了将潜在表示的值范围调整到合适的区间
                ...
                # 对噪声潜变量进行适当的缩放 ： sigmas 是根据时间步算好的，这样的缩放可以让噪声在不同时间步有相同的统计特性
