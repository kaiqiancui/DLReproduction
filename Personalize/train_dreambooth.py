...
def log_validation(
    ...
    #准备生成参数
    if args.pre_compute_text_embeddings: #是否使用预计算的文本嵌入
        pipeline_args = {
...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        class_num=None,         # 类别图像数量（可选）
        size=512,              # 图像大小
        center_crop=False,      # 是否中心裁剪
        encoder_hidden_states=None,  # 编码器隐藏状态
        class_prompt_encoder_hidden_states=None,  # 类别提示词编码器隐藏状态
    ...
    def __len__(self):
        return self._length  # 返回数据集长度（实例图像数量和类别图像数量的最大值）
...
def collate_fn(examples, with_prior_preservation=False):
    #数据批处理函数，将多个样本组合成一个批次
    
    has_attention_mask = "instance_attention_mask" in examples[0]

    # 从每个样本中提取实例提示词ID和图像数据
    ...
    #处理先验保持数据 当启用先验保持时，将类别数据添加到批次中
    #这个是论文实现的时候的核心，他提出了这个loss
    #传统的前向传播：第一次处理实例数据，第二次处理类别数据，然后计算总损失
    #dreambooth是希望让这两个东西同时进行前向传播
...
def model_has_vae(args):
    ...
    if os.path.isdir(args.pretrained_model_name_or_path):
        #如果是本地路径，直接拼接完整的配置文件路径
        config_file_name = os.path.join(args.pretrained_model_name_or_path, config_file_name)
        
        return os.path.isfile(config_file_name)
    else:
        #如果不是本地路径，如果是仓库路径：获取仓库中所有文件的信息
...
#文本编码的过程：将标记化之后的文本转化为向量表示
def encode_prompt(
    text_encoder, 
    input_ids, #输入的token IDs
    attention_mask, 
    text_encoder_use_attention_mask=None #是否使用注意力掩码
    ):
    #将输入数据移动到编码器所在的位置
