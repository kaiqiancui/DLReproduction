...
class FullAdapter(nn.Module):
    ...
    def __init__(
        self,
        in_channels: int = 3,
        channels: List[int] = [320, 640, 1280, 1280], # 每个下采样块输出的通道数列表
        num_res_blocks: int = 2,
        downscale_factor: int = 8,
    ):
        super().__init__()

        in_channels = in_channels * downscale_factor**2 # 通道数加个平方，因为PixelUnshuffle会增加通道数
